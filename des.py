...
def s_box_substitution(block):
    ...
    for i in range(len(sub_blocks)):
        ...
        col = int(f"{''.join(map(str, sub_blocks[i][1:5]))}", 2)
        val = S_BOX[i][row][col]
...
def des_block(block, keys, encrypt=True):
    ...
    final_block = permute(combined_block, FP)
    return final_block

def des_encrypt(plain_text, key):
    keys = key_schedule(key)
    cipher_text = des_block(plain_text, keys, encrypt=True)
    return cipher_text

def des_decrypt(cipher_text, key):
    keys = key_schedule(key)
    plain_text = des_block(cipher_text, keys, encrypt=False)
    return plain_text
...
def bit_array_to_str(array):
    res = ''.join([chr(int(''.join([str(x) for x in _byte]), 2)) for _byte in nsplit(array, 8)])
    return res
...
# Example usage:
plain_text =input("Enter your massage: ")
key = input("Your 64-bit key: ")  # Must be 8 characters for 64 bits
plain_text_bits = str_to_bit_array(plain_text)
key_bits = str_to_bit_array(key)

# Encrypt
cipher_text_bits = des_encrypt(plain_text_bits, key_bits)
cipher_text = bit_array_to_str(cipher_text_bits)

# Decrypt
decrypted_text_bits = des_decrypt(cipher_text_bits, key_bits)
decrypted_text = bit_array_to_str(decrypted_text_bits)
